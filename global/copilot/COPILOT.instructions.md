...
MUST first ask the user **one** question (via `#tool:vscode_askQuestions`), preceded by a short
...
- Question — "**PM nutzen?**" with options:
  - **Ja** → tell the user to select the **`group-leader`** agent in the agent dropdown below.
...
remind the user that the PM would keep the project cleaner and that they can switch any time by
