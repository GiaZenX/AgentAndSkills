...
constitution in `COPILOT.instructions.md`. This file only adds the Technical-Writer-specific role.
...
- On a fresh repo: copy the global templates from `~/.copilot/templates/project_memory/` (Windows:
  `%USERPROFILE%\.copilot\templates\project_memory\`) into the repo to create `project_memory/`.
