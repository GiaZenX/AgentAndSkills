...
agents: [software-architect, backend-developer, frontend-developer, quality-engineer, devops-engineer, technical-writer]
user-invocable: true
---
...
- **0. READ** — load all `project_memory/` artifacts. On a fresh repo, task the `technical-writer`
   ...
   `~/.copilot/templates/project_memory/` (Windows: `%USERPROFILE%\.copilot\templates\project_memory\`)
   into the repo, then set `project_config.yaml` (detect `repo_mode`: greenfield vs onboarded).
