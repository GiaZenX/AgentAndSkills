...
anytime and that you may occasionally remind them. Then ask **one** question
...
3. **Hand off to the PM:** tell the user to **select `project-manager` in the agent dropdown** below.
   Once selected it stays active, so all further work runs through the PM. Restate plainly:
   "Ab jetzt läuft alles über den Project Manager — bitte unten `project-manager` auswählen."
...
**not** write code as the default agent. Tell the user to select `project-manager` in the dropdown:
"Dieses Repo arbeitet über den Project Manager — bitte unten `project-manager` auswählen." This holds
across sessions, so a forgotten agent selection can never lead to unstructured edits.
